...
contract Crowdfunding2 {
    ...
    struct Campaign {
        ...
        bytes proof; // Store proof of World ID verification
        string nullifierHash; // Store World ID nullifier hash
    }
    ...
    struct Transactions {
        ...
        uint256 timestamp;
        uint256 milestoneId;
        bytes proof; // Store proof of World ID verification for donation
        string nullifierHash; // Store World ID nullifier hash for donation
    }
    ...
    mapping(string => bool) public usedNullifierHashes; // Prevent double World ID submissions
    ...
    function createCampaign(
        ...
        bytes memory _proof,  // World ID proof
        string memory _nullifierHash // World ID nullifier hash
        ...
        campaign.proof = _proof;
        campaign.nullifierHash = _nullifierHash;
    ...
    function donateToMilestone(
        uint256 _campaignId,
        uint256 _milestoneId,
        uint256 _customTimestamp,
        bytes memory _proof, // World ID proof
        string memory _nullifierHash // World ID nullifier hash
    ) public payable {
        ...
        campaign.transactions.push(Transactions(msg.sender, msg.value, currentTimeStamp, _milestoneId, _proof, _nullifierHash));
